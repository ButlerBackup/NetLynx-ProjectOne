...
public class WebRequestAPI {
	...
	public ArrayList<Number> getHistoryNumbersOnly(ArrayList<HashMap<String, String>> data) {
	...
	public String requestPin() {
		SoapObject rpc = new SoapObject(Consts.NAMESPACE, "GetDevice"); // create new soap object
		rpc.addProperty("UDID", "1234567890"); // set parameter
		Log.e("UDID", "1234567890");
		rpc.addProperty("deviceID", "51235000031111"); // set parameter
		Log.e("DEVICEID", "51235000031111");
		SoapSerializationEnvelope envelope = new SoapSerializationEnvelope(SoapEnvelope.VER11);
		envelope.dotNet = true;
		envelope.setOutputSoapObject(rpc);
		HttpTransportSE ht = new HttpTransportSE("http://192.168.10.8/wsIOT/TH.asmx"); // set base link
		ht.debug = true;
		try {
			// //Log.e("WebRequest", "TRY!");
			ht.call("http://NetLynxTech.com/GetDevice", envelope); // call web request
			System.err.println(ht.responseDump);
			SoapObject result = (SoapObject) envelope.getResponse(); // get response
			// Log.e("RESULT", result.toString());
			// Log.e("COUNT", result.getPropertyCount() + "");
